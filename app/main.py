...
# ---------------------------------------------------------------------------
# Telegram-only access enforcement (always registered, unconditionally)
# ---------------------------------------------------------------------------
...
async def enforce_telegram_access(request: Request, call_next):
    ...
    # Webhook endpoints bypass this check (they use signature verification)
    ...
    if path in public_paths or any(path.startswith(p) for p in webhook_prefixes + admin_prefixes):
        return await call_next(request)

    # For API requests: MUST have a valid Telegram init_data header
    if path.startswith("/api/"):
        authorization = request.headers.get("authorization", "")
        init_data_header = request.headers.get("init-data", "")
        if not (authorization.startswith("tma ") or init_data_header):
            return JSONResponse(
                status_code=403,
                content={"detail": "This service is only accessible from Telegram"},
            )
    else:
        # For non-API (static/frontend) requests: check Telegram referer or user-agent
