...
export default function Store() {
  ...
  const handleDeposit = () => {
    ...
    // Create a temporary plan for the deposit amount
    const tempPlan = {
      plan_name: `Deposit_${amount}`,
      price_usd: amount
    }
    handleBuy(tempPlan)
  }
  ...
  return (
    <div className="px-4 py-5 space-y-5">
        ...
        <>
          ...
          {!renewState?.renewUuid && (
            <div className="bg-slate-800 rounded-xl ring-1 ring-slate-700 p-4 space-y-3">
              <div className="flex items-center gap-2">
                <DepositIcon />
                <h2 className="text-slate-300 font-semibold text-sm">Custom Deposit</h2>
              </div>
              <p className="text-slate-400 text-xs">Enter the amount of USDT you want to deposit</p>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 disabled:text-slate-500 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors"
                >
                  Deposit
                </button>
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-slate-300 font-semibold text-sm">
              {renewState?.renewUuid ? 'Select Plan to Renew' : 'Quick Charge Plans'}
            </h2>
          </div>
      ...
      {!renewState?.renewUuid && activeTab === 'withdrawal' && (
        <div className="bg-slate-800 rounded-xl ring-1 ring-slate-700 p-6 text-center space-y-4">
          <WithdrawIcon />
          <div>
            <h3 className="text-white font-semibold text-base mb-2">Withdraw Balance</h3>
            <p className="text-slate-400 text-sm mb-1">
              To withdraw your balance, please submit a support ticket with the withdrawal category.
            </p>
            <p className="text-slate-400 text-sm">
              Include your USDT wallet address and network (TRC20, BEP-20, or TON).
            </p>
          </div>
          <button
